...
function RequiredTestsEditor({ items, setItems }) {
  function update(i, field, value) {
    setItems(items.map((item, index) => (index === i ? { ...item, [field]: value } : item)));
  }
  function add() {
    setItems([...items, { name: "", instructions: "" }]);
  }
  function remove(i) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <span className="form-label mb-0">Tests</span>
        <button type="button" onClick={add} className="text-xs font-semibold text-primary">+ Add test</button>
      </div>
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="grid grid-cols-7 gap-2 items-center">
            <input aria-label={`Test #${i + 1} name`} className="form-input col-span-3" placeholder="Test name" value={item.name} onChange={(e) => update(i, "name", e.target.value)} />
            <input aria-label={`Test #${i + 1} instructions`} className="form-input col-span-3" placeholder="Instructions (optional)" value={item.instructions} onChange={(e) => update(i, "instructions", e.target.value)} />
            <button type="button" onClick={() => remove(i)} aria-label={`Remove test #${i + 1}`} className="text-xs text-rejected-text col-span-1">Remove</button>
          </div>
        ))}
        {items.length === 0 && <p className="text-xs text-ink-3">No tests prescribed.</p>}
      </div>
    </div>
  );
}

function AdditionalBillingEditor({ items, setItems }) {
  function update(i, field, value) {
    setItems(items.map((item, index) => (index === i ? { ...item, [field]: value } : item)));
  }
  function add() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <span className="form-label mb-0">Billable Items</span>
        <button type="button" onClick={add} className="text-xs font-semibold text-primary">+ Add item</button>
      </div>
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="grid grid-cols-7 gap-2 items-center">
            <input aria-label={`Additional billing item #${i + 1}`} className="form-input col-span-4" placeholder="Product or service" value={item.name} onChange={(e) => update(i, "name", e.target.value)} />
            <input aria-label={`Additional billing item #${i + 1} price`} type="number" min="0" step="0.01" className="form-input col-span-2" placeholder="Price (₹)" value={item.amount} onChange={(e) => update(i, "amount", e.target.value)} />
            <button type="button" onClick={() => remove(i)} aria-label={`Remove additional billing item #${i + 1}`} className="text-xs text-rejected-text col-span-1">Remove</button>
          </div>
        ))}
        {items.length === 0 && <p className="text-xs text-ink-3">No additional charges added.</p>}
      </div>
    </div>
  );
}
...
export default function ConsultationForm({ visit, patient, consultation }) {
  ...
  const [requiredTests, setRequiredTests] = useState(consultation?.requiredTests || []);
  const [additionalBilling, setAdditionalBilling] = useState(consultation?.additionalBilling || []);
  ...
  async function handleComplete(e) {
    ...
    const billableItems = additionalBilling.filter((item) => item.name.trim() !== "" || item.amount !== "");
    if (billableItems.some((item) => !item.name.trim() || item.amount === "" || !Number.isFinite(Number(item.amount)) || Number(item.amount) < 0)) {
      setError("Each additional billing item needs a product or service and a valid price.");
      return;
    }
    setLoading(true);
    try {
      // Drop medicine or test rows left blank (e.g. an add button clicked but never
      // filled in) instead of persisting empty entries.
      ...
      await completeConsultation(visit.id, {
        ...
        requiredTests: withoutBlankRows(requiredTests),
        additionalBilling: billableItems.map((item) => ({ name: item.name.trim(), amount: Number(item.amount) })),
      });
  ...
  return (
    <form onSubmit={handleComplete} className="space-y-6">
      ...
      <div className="card p-5 space-y-3">
        <h2 className="font-semibold text-ink mb-3">D. Required Tests (Optional)</h2>
        <RequiredTestsEditor items={requiredTests} setItems={setRequiredTests} />
      </div>

      <div className="card p-5 space-y-3">
        <h2 className="font-semibold text-ink mb-3">E. Additional Billing</h2>
        <AdditionalBillingEditor items={additionalBilling} setItems={setAdditionalBilling} />
      </div>
