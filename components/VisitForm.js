...
// `onBook` and `onSuccess` default to the original patient-self-booking behavior
// (createVisit + redirect to the patient dashboard) so every existing caller of this
// component is unaffected. Passing a different `onBook` (e.g. createVisitByStaff) and
// `onSuccess` (e.g. show the new queue token inline instead of navigating away) is
// what lets components/PatientSearchAndRegister.js reuse this same form for the
// doctor/registration-desk walk-in booking step instead of duplicating it.
export default function VisitForm({ patient, doctor, onBook = createVisit, onSuccess }) {
