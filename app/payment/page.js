...
import { getVisit, getPaymentForVisit, getDoctor, getConsultationForVisit } from "@/lib/db";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setConsultation(await getConsultationForVisit(visitId));
      } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md">
        ...
        {!payment ? (
          ...
          <PaymentPanel visit={visit} payment={payment} doctor={doctor} consultation={consultation} />
        )}
      </div>
    </div>
  );
