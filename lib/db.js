...
// Ordered by name so the list is stable between loads. Without an explicit order
// Postgres is free to return rows in whatever physical order it likes, and that order
// actually shifts in practice -- every Available/Busy toggle rewrites a doctors row and
// can move it. app/visit/new/page.js used to just take doctors[0], so on a clinic with
// more than one doctor the same patient could be shown (and booked against) a different
// doctor on each visit to the page.
export async function getDoctors() {
  const data = unwrap(await supabase.from("doctors").select("*").order("name"));
...
export async function updateDoctorProfile(doctorId, {
  ...
  // Only written when the caller actually passed it, so a profile save from a form
  // that doesn't manage the QR can never blank out an already-uploaded one.
  if (qrCodeUrl !== undefined) {
    row.qr_code_url = qrCodeUrl || null;
  }
  const data = unwrap(
...
// Also stamps mode as "online": uploading a payment screenshot *is* the online
// payment path, and a patient who booked as cash can settle online from the payment
// screen. It has to be set in this same update rather than a separate one, because
// payments_update_own_patient (see supabase.sql) only permits a patient write whose
// resulting status is 'awaiting-verification' -- a standalone mode change would be
// rejected by RLS.
export async function uploadPaymentScreenshot(visitId, screenshotDataUrl) {
  const data = unwrap(
    await supabase
      .from("payments")
      .update({ screenshot_data_url: screenshotDataUrl, mode: "online", status: "awaiting-verification" })
      .eq("visit_id", visitId)
      .select()
      .maybeSingle()
  );
...
// ---------- Admin (account provisioning only) ----------

export async function getAdminByFirebaseUid(uid) {
  const data = unwrap(await supabase.from("admins").select("*").eq("firebase_uid", uid).maybeSingle());
  return rowToAdmin(data);
}

// The doctors row for an account the admin page just created in Firebase. Written as a
// plain insert rather than an RPC: doctors_insert_admin already restricts this to
// admins, and there is nothing here a security-definer function would need to hide.
// Everything else about the profile (fee, timings, QR, address) is left at its default
// for the doctor to fill in themselves from Doctor Dashboard -> Profile.
export async function createDoctorRow({ firebaseUid, name, email }) {
  const { data, error } = await supabase
    .from("doctors")
    .insert({ firebase_uid: firebaseUid, name, email })
    .select()
    .single();
  if (error) {
    if (error.code === "23505") {
